...
export function useProviderSetup(opts?: { onSaved?: () => void }): ProviderSetupState {
  ...
  const runTestAndSave = async (): Promise<boolean> => {
    ...
    const res = await verifyProvider(sel, fields).catch(() => ({ ok: false, error: t("providers.unreachable") }));
    if (!res.ok) {
      setVerify({ state: "error", msg: res.error || t("providers.couldntVerify") });
      return false;
    }
  ...
  const statusFor = (p: ProviderInfo, o?: { lastUsed?: boolean }) => {
    if (p.configured && p.needs_key) {
      const used = o?.lastUsed ? relTime(t, p.last_used_at) : null;
      return (
        <span className="block text-[11.5px] text-ok font-medium truncate">
          {t("providers.connected")}
          {used ? <span className="text-muted font-normal">{t("providers.lastUsed", { used })}</span> : ""}
        </span>
      );
    }
    if (!p.needs_key)
      return (
        <span className="block text-[11.5px] text-faint truncate">
          {keylessOk.has(p.name) ? <span className="text-ok font-medium">{t("providers.running")}</span> : t("providers.keyless")}
        </span>
      );
    return <span className="block text-[11.5px] text-faint truncate">{t("providers.notSetUp")}</span>;
  };
      ...
      <div className="flex gap-2">
        <div className="relative flex-1 min-w-0">
          ...
          {ps.savedState && testable && (
            ...
            >
              {info?.needs_key ? t("providers.testedAndSaved") : t("providers.detected")}
            </span>
          )}
        </div>
        {testable && (
          <button
            ...
            data-testid={`${tp}-test`}
          >
            {ps.verify.state === "testing" ? "…" : info?.needs_key ? t("providers.test") : t("providers.detect")}
          </button>
        )}
      </div>
    ...
    <div>
            ...
            <div className="mt-3.5 flex items-center justify-between gap-3 border-t border-line pt-3">
              {ps.savedState ? (
                <span className="text-[11.5px] font-medium text-ok" data-testid={`${tp}-saved-pill`}>
                  {t("providers.testedAndSaved")}
                </span>
              ) : (
                <span className="text-[11.5px] text-faint">{t("providers.checkSaves")}</span>
              )}
              <button
                ...
                data-testid={`${tp}-test`}
              >
                {ps.verify.state === "testing" ? "…" : t("providers.testAndSave")}
              </button>
            </div>
          </div>
        </div>
      ...
      {info?.needs_key && KEY_HELP[sel] && (
        <p className="text-[11.5px] text-faint mt-2">
          {t("providers.noKeyYet")}{" "}
          <button
            className="text-muted underline decoration-line underline-offset-2 hover:text-ink"
            onClick={() => openExternal(KEY_HELP[sel].url)}
          >
            {t("providers.createOneAt", { label: KEY_HELP[sel].label })}
          </button>{" "}
          {t("providers.takesAMinute")}
        </p>
      )}
      {info && !info.needs_key && (
        <p className="text-[11.5px] text-faint mt-2">
          {t("providers.noKeyNeededOllama")}{" "}
          <button
            className="text-muted underline decoration-line underline-offset-2 hover:text-ink"
            onClick={() => openExternal("https://ollama.com/download")}
          >
            {t("providers.installOllama")}
          </button>
        </p>
      )}
