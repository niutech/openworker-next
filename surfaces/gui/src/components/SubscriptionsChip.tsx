  ...
  return (
    <div className="relative flex-1 min-w-0" ref={wrap}>
      <input
        ...
        placeholder={t("channelPicker.placeholderLink")}
        value={display}
  ...
  return (
    <div className="sub-chip-wrap" ref={ref}>
      <button
        ...
        title={t("subscriptions.sessionTitle")}
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <div className="sub-pop" onMouseDown={(e) => e.stopPropagation()}>
          <div className="sub-pop-head">{t("subscriptions.sessionTitle")}</div>
          {channels.length === 0 ? (
            <div className="dim sub-pop-empty">{t("subscriptions.empty")}</div>
          ) : (
            channels.map((c) => {
