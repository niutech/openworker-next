...
function approvalChip(resolved: ApprovalDecision | undefined, t: TFunction) {
  if (resolved === "deny")
  ...
  return (
    <span
      ...
      title={resolved ? t("transcript.approvedWith", { detail: resolved.replace(/_/g, " ") }) : t("transcript.approved")}
    >
      ...
      <div className="group flex items-baseline gap-2 px-2 py-0.5 rounded-lg hover:bg-paper" data-testid="turn-step">
        ...
        <LineText
          line={
            ...
            tool.name === "load_skill" && tool.preview?.includes('"error"')
              ? { pre: t("transcript.triedSkill"), obj: String(tool.args?.name ?? ""), post: t("transcript.notAvailable") }
        ...
        {!!tool.standingRule && (
          <span
            ...
            title={t("transcript.autoAllowedTitle", { rule: tool.standingRule })}
          >
            {t("transcript.autoAllowed")}
          </span>
        )}
        {!!tool.hidden && (
          <span
            ...
            title={t("transcript.hiddenTitle")}
          >
            {t("transcript.hiddenCount", { count: tool.hidden })}
          </span>
        )}
...
function TurnGroup({
  ...
  const stepsLabel = t("transcript.steps", { count: nSteps });
  ...
  return (
    <details className="stepgroup" open={open}>
      ...
      >
        ...
        <span>
          <span>{running ? t("transcript.runningSteps", { count: nSteps }) : stepsLabel}</span>
          {declined > 0 && (
            <>
              {" · "}
              <span className="text-danger" data-testid="stepgroup-declined">
                {t("transcript.declinedCount", { count: declined })}
              </span>
            </>
          )}
          {hiddenTotal > 0 && (
            <>
              ...
              <span className="text-warnInk" data-testid="stepgroup-hidden">
                {t("transcript.hiddenByFilters", { count: hiddenTotal })}
              </span>
            </>
          )}
        </span>
          ...
          case "dirreq":
            ...
            return (
              <div className="approval-inline" key={bi}>
                ...
                <span>{item.resolved === "granted" ? t("transcript.grantedFolderAccess") : t("transcript.declinedFolderAccess")}</span>
            ...
            return (
              <div className="bubble-assistant" key={bi}>
                ...
                <div className="approval-inline">
                  ...
                  <span>{item.resolved === "approved" ? t("transcript.planApproved") : t("transcript.sentBackWithFeedback")}</span>
                </div>
              </div>
            );
          case "notice":
