...
export function AccessSection({
  ...
  const addChannel = async () => {
    ...
    if (!r.ok) {
      setAddErr(r.error || t("access.couldNotAddChannel"));
      return;
    }
    setAddErr(null);
    setDraft("");
  ...
  const folderPart = projectScoped
    ...
    : roots.length > 0
      ? t("access.folderCount", { count: roots.length })
      : null;
              ...
              <div>
                    ...
                    <div className="flex items-center gap-2 py-1" key={c.connector}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {byName[c.connector]?.channels && (
                          ...
                          >
                            {t("access.channelsCount", { count: channelsOf(c.connector).length })}
                      ...
                      <Toggle
                        ...
                        title={t("access.enabledForSession")}
                      />
                    </div>
                  ))}
                </div>
                {connected.length > 0 && (
                  <p className="text-[10.5px] text-faint mt-1 leading-snug">
                    {t("access.offMutesPrefix")} <b>{t("access.thisSessionOnly")}</b>
                    {t("access.offMutesSuffix")}
                  </p>
                )}
                      ...
                      <div className="flex items-center gap-2 py-1" key={r.connector}>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 text-[12.5px] font-medium leading-tight">
                            ...
                            {r.tier === "core" && <span className={TAG_CORE}>{t("access.core")}</span>}
                          </div>
...
function ConnectInline({
  ...
  return (
    <div>
      ...
      >
        <Icon name="arrowLeft" size={13} /> {t("access.connectTitle", { title: c.title })}
      </button>
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        {t("access.connectingMakes", { title: c.title })}
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      <button
        ...
        aria-label={t("access.backToSources")}
      >
        <Icon name="arrowLeft" size={13} /> {t("access.channelsLabel", { label })}
      </button>
      <div className={`${SEC_H} mb-1.5`}>{t("access.subscribedChannels", { count: channels.length })}</div>
      {channels.length === 0 ? (
        <div className="text-[12px] text-faint py-0.5">
          {t("access.notListening", { label })}
        </div>
      ) : (
        <div className="space-y-1">
          {channels.map((s) => (
            <div className="flex items-center gap-1.5 py-1" key={s.channel}>
              ...
              {s.collision && (
                <span
                  ...
                  title={t("access.collisionWarning")}
                >
                  ⚠
                </span>
              )}
