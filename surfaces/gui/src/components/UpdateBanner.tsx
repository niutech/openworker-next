...
export function UpdateBanner() {
  ...
  return (
    ...
    >
      <div className="text-[13px] font-semibold">{t("common.updateAvailable")}</div>
      <div className="text-[12px] text-muted mt-0.5">
        {t("updateBanner.readyToInstall", { version: update.version })}
      </div>
      {phase === "error" && (
        <div className="text-[11.5px] text-warnInk mt-1.5">
          {t("updateBanner.couldNotInstall")}
        </div>
      )}
      <div className="flex items-center gap-2 mt-2.5">
        ...
        >
          {busy ? t("updateBanner.downloading") : t("updateBanner.restartToUpdate")}
        </button>
        <button
