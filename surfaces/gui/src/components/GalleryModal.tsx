...
export function GalleryModal({
  ...
  const openDetail = async (slug: string) => {
    ...
    setDetail(d ?? { ok: false, error: t("gallery.couldNotLoadDetails") });
  };
      ...
      <div className="space-y-2">
            ...
            >
              ...
              <div className="shrink-0 flex items-center">
                {isInstalled ? (
                  ...
                  <span className="text-[12.5px] text-accent">{t("gallery.viewAndInstall")}</span>
                )}
              </div>
            </div>
          );
        ...
        {visible.length === 0 && !unavailable && (
          <div className="text-[12.5px] text-muted py-4">
            {source === "team"
              ? t("gallery.nothingShared")
              : q
              ? t("gallery.noMatch")
              : t("gallery.noPersonasPublished")}
          </div>
        )}
      </div>
  ...
  const detailView = detailSlug && (
    <div data-testid="gallery-detail">
      ...
      {!detail ? (
        <div className="text-[12.5px] text-muted">{t("common.loading")}</div>
      ) : !detail.ok || !card ? (
        <div className="text-[12.5px] text-danger">{detail.error || t("gallery.couldNotLoadDetails")}</div>
      ) : (
            ...
            <div className="shrink-0">
              {installed.has(detailSlug) ? (
                ...
                <button className={BTN_ACCENT} onClick={() => install(detailSlug)} disabled={busy}>
                  {busy ? t("gallery.installing") : t("common.install")}
                </button>
              )}
            </div>
          </div>
          ...
          {caps && (
            <div className={CARD + " p-4"} data-testid="gallery-capabilities">
              ...
              <div className="text-[12px] text-faint mb-3">
                {t("gallery.readByParser")}
              </div>
              <div className="space-y-2 text-[12.5px]">
                <div>
                  <span className="text-muted">{t("gallery.tools")}</span>
                  {caps.tools.join(", ") || t("common.none")}
                  {caps.risk.length > 0 && (
                    <span className="text-faint">{t("gallery.riskList", { list: caps.risk.join(", ") })}</span>
                  )}
                </div>
                <div>
                  <span className="text-muted">{t("gallery.permissions")}</span>
                  {caps.recommended_mode} {t("gallery.mode")}
                  {caps.messaging ? t("gallery.canUseMessaging") : ""}
                  {caps.mcp.length > 0 ? t("gallery.mcpList", { list: caps.mcp.join(", ") }) : ""}
                </div>
                      ...
                      {detail.recommends!.map((r) => (
                        <div key={r.kind + r.ref} className="flex items-baseline gap-2">
                          <span className={CHIP + " inline-flex items-center gap-1 shrink-0"}>
                            ...
                            {r.ref}
                            {r.tier === "core" ? t("gallery.coreSuffix") : ""}
                          </span>
        ...
        <div className="px-5 pt-4 pb-3 border-b border-line flex items-center gap-3 shrink-0">
          ...
          {cloud?.signed_in && !detailSlug && (
            <input
              ...
              placeholder={t("gallery.searchPersonas")}
        ...
        <div className="overflow-y-auto hairline-scroll p-5">
          {loading ? (
            <div className="space-y-2" data-testid="gallery-loading" aria-busy="true">
              <div className="text-[12.5px] text-muted mb-3">{t("gallery.loadingGallery")}</div>
              {[0, 1, 2].map((i) => (
            ...
            <div className={CARD + " p-5 flex items-center gap-4"} data-testid="gallery-signin">
              <div className="min-w-0 flex-1">
                <div className="font-semibold text-[14px] mb-1">{t("gallery.signInToBrowse")}</div>
                <div className="text-[12.5px] text-muted leading-relaxed">
                  {t("gallery.signInDesc")}
                </div>
              </div>
              <button className={BTN_ACCENT} onClick={signIn} disabled={signingIn}>
                {signingIn ? t("cloud.checkBrowser") : t("setup.signIn")}
              </button>
            </div>
          ) : detailSlug ? (
            detailView
