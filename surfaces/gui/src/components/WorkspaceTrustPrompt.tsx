...
export function WorkspaceTrustPrompt({
  ...
  const trust = async () => {
    ...
    setSaving(false);
    if (!result?.ok) {
      setError(result?.error || t("workspaceTrust.couldNotSave"));
      return;
    }
    onClose();
  };
  ...
  return (
    <div className="gate-overlay" role="dialog" aria-modal="true" aria-labelledby="workspace-trust-title">
      <div className="gate max-w-[560px]">
        ...
        <h2 id="workspace-trust-title">{t("workspaceTrust.trustCommands")}</h2>
        <p className="gate-sub">{t("workspaceTrust.description")}</p>
        ...
        <div className="gate-foot justify-end gap-2">
          <button className="btn" onClick={onClose} disabled={saving}>
            {t("workspaceTrust.keepAsking")}
          </button>
          <button className="btn primary" onClick={() => void trust()} disabled={saving}>
            {saving ? t("workspaceTrust.saving") : t("workspaceTrust.trustWorkspace")}
          </button>
        </div>
      </div>
    </div>
