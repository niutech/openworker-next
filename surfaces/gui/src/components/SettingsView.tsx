...
export function SettingsView({
  ...
  const SET_TABS: { key: SetTab; label: string; icon: "sliders" | "code" | "mic" | "archive" | "sparkle" | "book" }[] = [
    { key: "appearance", label: t("settings.general"), icon: "sliders" },
    { key: "models", label: t("settings.models"), icon: "code" },
    { key: "skills", label: t("settings.skills"), icon: "book" },
    { key: "voice", label: t("settings.voice"), icon: "mic" },
    { key: "memory", label: t("settings.memory"), icon: "archive" },
    { key: "personas", label: t("settings.personas"), icon: "sparkle" },
  ];
...
const voiceError = (error: unknown, t: TFunction) =>
  error instanceof Error ? error.message : typeof error === "string" ? error : t("settings.voiceGenericError");
...
function VoiceInputSection() {
  ...
  const remove = async () => {
    if (!window.confirm(t("settings.voiceDeleteConfirm"))) return;
    setError(null);
    try {
      publish(await deleteDictationModel());
      setTestTranscript("");
  ...
  const toggleTest = async () => {
    ...
    try {
      if (status.recording) {
        ...
        if (!transcript) throw new Error(t("settings.voiceNoSpeech"));
  ...
  return (
    <section>
      <PanelHead
        ...
        sub={t("settings.voiceInputSub")}
      />

      {!desktop ? (
        <div className={CARD + " p-4 text-[13px] text-muted"}>{t("settings.voiceDesktopOnly")}</div>
      ) : (
        <div className="space-y-4">
          <div className="rounded-xl border border-green-200 bg-green-50/70 px-4 py-3 text-[12.5px] text-green-800">
            <span className="font-medium">{t("settings.voicePrivateTitle")}</span> {t("settings.voicePrivateDesc")}
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{t("settings.voiceThisDevice")}</div>
                <div className="text-[12px] text-muted mt-1">{status?.device_summary || t("settings.voiceChecking")}</div>
              ...
              {status && (
                <span className={"text-[11.5px] px-2 py-1 rounded-full " + (status.supported ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600")}>
                  {status.supported ? t("settings.voiceCompatible") : t("settings.voiceUnsupported")}
                </span>
              )}
            </div>
            <div className="border-t border-line bg-paper/50 px-4 py-3 grid grid-cols-2 gap-3 text-[12px] text-muted">
              <div><span className="block text-ink font-medium">{t("settings.voiceRequirementsMac")}</span>{t("settings.voiceRequirementsMacDesc")}</div>
              <div><span className="block text-ink font-medium">{t("settings.voiceRequirementsWindows")}</span>{t("settings.voiceRequirementsWindowsDesc")}</div>
              <div><span className="block text-ink font-medium">{t("settings.voiceRequirementsMemory")}</span>{t("settings.voiceRequirementsMemoryDesc")}</div>
              <div><span className="block text-ink font-medium">{t("settings.voiceRequirementsProcessor")}</span>{t("settings.voiceRequirementsProcessorDesc")}</div>
            </div>
          </div>
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{t("settings.voiceModel")}</div>
                <div className="text-[12px] text-muted mt-0.5">
                  {status?.model_verified ? t("settings.voiceModelInstalled", { size: formatBytes(status.model_bytes) }) : t("settings.voiceModelNotInstalled", { size: formatBytes(status?.model_bytes || 147_964_211) })}
                </div>
              </div>
              {status?.model_verified ? (
                <>
                  <span className="text-[11.5px] px-2 py-1 rounded-full bg-green-50 text-green-700">{t("settings.voiceModelVerified")}</span>
                  <button className={BTN_BORDERED} onClick={() => void repair()}>{t("settings.voiceModelRepair")}</button>
                  <button className="text-[12px] text-red-600 px-2 py-2" onClick={() => void remove()}>{t("settings.voiceModelDelete")}</button>
                </>
              ) : downloading ? (
                <button className={BTN_BORDERED} onClick={() => void cancelDownload()}>{t("settings.voiceModelCancel")}</button>
              ) : phase === "verifying" ? (
                <span className="text-[12px] text-muted">{t("settings.voiceModelVerifying")}</span>
              ) : (
                <button className={BTN_ACCENT} disabled={!status?.supported} onClick={() => void download()}>{t("settings.voiceModelDownload")}</button>
              )}
            </div>
            {downloading && (
          ...
          <div className={CARD}>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13.5px] font-medium">{t("settings.voiceMicTest")}</div>
                <div className="text-[12px] text-muted mt-0.5">
                  {ready ? t("settings.voiceMicTestReady") : t("settings.voiceMicTestNotReady")}
                </div>
              </div>
              {ready && <span className="text-[11.5px] px-2 py-1 rounded-full bg-green-50 text-green-700">{t("settings.voiceMicTestReadyBadge")}</span>}
              <button className={BTN_BORDERED} disabled={!status?.supported || !status?.model_verified || phase === "transcribing"} onClick={() => void toggleTest()}>
                {status?.recording ? t("settings.voiceMicTestStop") : phase === "transcribing" ? t("settings.voiceMicTestTranscribing") : ready ? t("settings.voiceMicTestTestAgain") : t("settings.voiceMicTestButton")}
              </button>
            </div>
            {status?.recording && <div className="border-t border-line px-4 py-3 text-[12px] text-accent" role="status">{t("settings.voiceMicListening")}</div>}
            {testTranscript && <div className="border-t border-line bg-paper/50 px-4 py-3 text-[13px]">{t("settings.voiceTranscriptPrefix", { transcript: testTranscript })}</div>}
          </div>
    ...
    <section>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[13.5px] font-medium">{t("settings.personasBrowseGallery")}</span>
          <span className="block text-[12px] text-muted">
            {t("settings.personasBrowseGalleryDesc")}
          </span>
        </span>
        <span className="text-[12.5px] text-accent shrink-0">{t("settings.personasOpen")}</span>
      </button>
      {galleryOpen && (
        <GalleryModal
  ...
  return (
    <section>
        ...
        <div className="seg mt-2.5" role="radiogroup" aria-label={t("settings.appearance")}>
          {(["light", "dark", "auto"] as const).map((p) => (
            <button key={p} className={p === theme ? "active" : ""} onClick={() => setTheme(p)}>
              {p === "light" ? t("settings.themeLight") : p === "dark" ? t("settings.themeDark") : t("settings.themeAuto")}
            </button>
          ))}
        </div>
        <div className={FIELD_HELP}>{t("settings.themeAutoDesc")}</div>
      </div>
        ...
        <div className={CARD + " p-4"}>
          ...
          <label className="flex items-start gap-3 py-2">
            ...
            <span>
              ...
              <span className="block text-[12px] text-muted">{t("settings.openAtLoginDesc")}</span>
            </span>
          </label>
          <label className="flex items-start gap-3 py-2">
            ...
            <span>
              ...
              <span className="block text-[12px] text-muted">{t("settings.keepAwakeDesc")}</span>
            </span>
          </label>
        </div>
      )}
      ...
      <div className={CARD + " p-4 mt-4"}>
        ...
        <div className={FIELD_HELP}>{t("settings.runSetupAgainHelp")}</div>
      </div>
    </section>
  );
}
...
function TrustedWorkspacesCard() {
  ...
  const revoke = async (path: string) => {
    if (!window.confirm(t("settings.trustedRevokeConfirm", { path }))) return;
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="trusted-workspaces-card">
      ...
      <div className={FIELD_HELP}>
        {t("settings.trustedWorkspacesHelp")}
      </div>
      {workspaces === null ? (
        <div className="text-[12px] text-muted mt-3">{t("settings.trustedWorkspacesLoading")}</div>
      ) : workspaces.length === 0 ? (
        <div className="text-[12px] text-muted mt-3">{t("settings.trustedWorkspacesNone")}</div>
      ) : (
        <div className="mt-3 divide-y divide-line">
          {workspaces.map((workspace) => (
            <div key={workspace.workspace} className="py-2.5 flex items-start gap-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11.5px] text-muted mt-0.5">
                  {workspace.requested_commands.length
                    ? t("settings.trustedWorkspacesCommands", { count: workspace.requested_commands.length })
                    : t("settings.trustedWorkspacesNoCommands")}
                  {!workspace.exists ? t("settings.trustedWorkspacesUnavailable") : ""}
                </div>
              </div>
              <button
              ...
              >
                {t("settings.trustedWorkspacesRevoke")}
              </button>
            </div>
          ))}
        </div>
    ...
    <span className="inline-flex items-center gap-2.5">
      {state === "found" ? (
        <button className={BTN_BORDERED} onClick={install} data-testid="settings-update-install">
          {t("settings.updateFound", { version })}
        </button>
      ) : (
        <button
        ...
        >
          {state === "checking" ? t("settings.checking") : t("settings.checkForUpdates")}
        </button>
      )}
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="token-savings-card">
      ...
      <div className="seg mt-2" role="radiogroup" aria-label={t("settings.pdfFallback")} data-testid="pdf-fallback">
        ...
        >
          {t("settings.pdfFallbackText")}
        </button>
        <button
          className={pdf.pdf_fallback === "images" ? "active" : ""}
          onClick={() => save({ pdf_fallback: "images" })}
        >
          {t("settings.pdfFallbackImages")}
        </button>
      </div>
      <div className={FIELD_HELP}>
        {t("settings.pdfFallbackHelp")}
      </div>

      <div className="mt-3 flex items-center gap-5">
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{t("settings.pdfMaxPages")}</span>
          <input
            type="number"
            min={1}
            max={100}
        ...
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{t("settings.pdfMaxMb")}</span>
          <input
            type="number"
            min={1}
            max={10}
          ...
          />
          <span className="text-[12.5px] text-muted">{t("settings.pdfMaxMbUnit")}</span>
        </label>
      </div>
      <div className={FIELD_HELP}>
...
function CompactionCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="compaction-card">
      ...
      <div className={FIELD_HELP}>
        {t("settings.contextCompactionHelp")}
      </div>

      <div className="mt-3 flex items-center gap-5 flex-wrap">
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{t("settings.compactAt")}</span>
          <input
            type="number"
            min={10}
            max={95}
          ...
          />
          <span className="text-[12.5px] text-muted">{t("settings.compactThresholdUnit")}</span>
        </label>
        <label className="flex items-center gap-2.5">
          <span className="text-[13px] text-ink">{t("settings.compactOrAt")}</span>
          <input
            type="number"
            min={10_000}
            max={2_000_000}
          ...
          />
          <span className="text-[12.5px] text-muted">{t("settings.compactCapUnit")}</span>
        </label>
      </div>
      <div className={FIELD_HELP}>
        {t("settings.compactCapHelp")}
      </div>
      ...
      <div className="mt-3 flex items-center gap-2.5">
        ...
        >
          <option value="">{t("settings.sessionOwnModel")}</option>
          {models.map((m) => (
      ...
      <div className={FIELD_HELP}>
        {t("settings.summarizerModelHelp")}
      </div>
    </div>
  );
}
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="context-bar-card">
      ...
      <label className="flex items-start gap-3 py-2">
        ...
        />
        <span>
          <span className="block text-[13px] text-ink">{t("settings.contextBarToggle")}</span>
          <span className="block text-[12px] text-muted">
            {t("settings.contextBarHelp")}
          </span>
        </span>
      </label>
    </div>
  ...
  return (
    <div className={CARD + " p-4 mb-4"}>
      ...
      <label className="flex items-center gap-3 mt-2.5">
        <span className="text-[13px] text-ink">{t("settings.sidebarConversationsPerCoworker")}</span>
        <input
          type="number"
          min={1}
          max={50}
      ...
      <div className={FIELD_HELP}>
        {t("settings.sidebarConversationsPerCoworkerHelp")}
      </div>
    </div>
  );
}
...
function FilesCard() {
  ...
  const saveScratch = async () => {
    ...
    if (res.ok) {
      setScratchMsg(t("settings.filesSaved"));
      refresh();
    } else {
      setScratchMsg(res.error || t("settings.filesError"));
    }
  };
      ...
      <div className={FIELD_LABEL}>{t("settings.files")}</div>
        <div className="flex items-center gap-2 mt-2.5">
          <input
            className={INPUT}
            type="text"
            placeholder={t("settings.filesPlaceholder")}
            value={scratchDraft}
            spellCheck={false}
            autoComplete="off"
          ...
          />
          {desktop && (
            <button className={BTN_BORDERED} onClick={browseScratch} title={t("settings.filesPickFolder")}>
              {t("settings.filesBrowse")}
            </button>
          )}
          <button className={BTN_ACCENT} onClick={saveScratch} disabled={!scratchDraft.trim()}>
            {t("settings.filesSave")}
          </button>
        </div>
      <div className={FIELD_HELP}>
