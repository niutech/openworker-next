...
function relayHealth(gh: GithubStatus | null, t: TFunction): { dot: string; text: string } {
  if (!gh) return { dot: "bg-ok", text: t("conn.liveManagedRelay") };
  if (!gh.signed_in)
    ...
    <div data-testid="github-installations">
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {t("conn.githubIntro")}
            {cloud?.signed_in ? "" : t("conn.githubCloudNeed")}
          </div>
        </div>
      )}
      ...
      {adding && (
        <AddConnectionModal
          c={c}
          cloud={cloud}
          title={t("conn.addInstallationTitle")}
      ...
      <div className={GRP_H + " flex items-center gap-2"}>
        <span>
          ...
          <span
            ...
            title={t("conn.installationId", { id: inst.installation_id })}
          >
            · {inst.repo_selection === "all" ? t("conn.allRepos") : t("conn.selectedRepos")}
          </span>
        </span>
        {!tokenOk && (
          <span className={TAG_WARN} data-testid={`token-warn-${inst.installation_id}`}>
            {t("conn.installationRevoked")}
          </span>
        )}
      </div>
      <div className={GRP}>
...
function DisconnectBtn({ id, busy, onClick }: { id: string; busy: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={t("conn.disconnectInstallationTitle")}
      onClick={onClick}
      disabled={busy}
    >
      {busy ? t("conn.disconnecting") : t("conn.disconnectInstallation")}
    </button>
  );
}
  ...
  return (
    <div className={ROW}>
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {allowed.length === 0 && (
          <span className="text-[12px] text-faint">{t("conn.nobodyYetGithub")}</span>
        )}
        {allowed.map((login) => (
          <span
            key={login}
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <button
        ...
        title={t("conn.allowDeliverMentionTitle")}
      ...
      <button
        ...
        title={t("conn.allowMentionTitle")}
        onClick={() => act("allow")}
      >
        {t("approval.allow")}
      </button>
