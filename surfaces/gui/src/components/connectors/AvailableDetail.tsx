...
export function AvailableDetail({
  ...
  return (
    <div data-testid="available-detail">
      ...
      {(c.access?.length ?? 0) > 0 && (
        <>
          <div className={GRP_H}>{t("access.title")}</div>
      ...
      {tools.length > 0 && (
        <>
          <div className={GRP_H}>{t("conn.tools")}</div>
          <div className={GRP}>
            <button
            ...
            >
              <span className="min-w-0 flex-1 text-muted">
                {t(tools.length === 1 ? "conn.toolsCount_one" : "conn.toolsCount_other", { count: tools.length })}
              </span>
            ...
            {showTools &&
              tools.map((tool) => (
                <div key={tool.name} className={ROW + " !min-h-[38px]"}>
                  ...
                  {tool.kind !== "read" && <span className={TAG_QUIET}>{t("conn.asksFirst")}</span>}
                </div>
              ))}
          </div>
        </>
