    ...
    <aside className={"right-rail" + (selected ? " artifact-mode" : "")}>
      {selected ? (
        ...
        <>
          ...
          {showArtifacts && (
          <RailSection
            title={t(artifacts.length ? "rightrail.artifactsCount" : "rightrail.artifacts", { count: artifacts.length })}
            open={open.artifacts}
              ...
              <>
                {artifacts.length > 0 && (
                  <button
                    ...
                    title={t("rightrail.showFolder")}
                  >
                    <Icon name="folder" size={13} />
                  </button>
                )}
                <button className="rail-mini-btn" onClick={(e) => { e.stopPropagation(); refreshArtifacts(); }} title={t("rightrail.refreshArtifacts")}><Icon name="refresh" size={13} /></button>
              </>
            }
          >
            {artifacts.length === 0 ? (
              <div className="rail-muted">{t("rightrail.noPreviewable")}</div>
            ) : (
...
function ProgressSummary({ running, toolNames, todo }: { running: boolean; toolNames: string[]; todo: TodoItem[] }) {
      ...
      <div className="rail-todo-list">
        ...
        {running && (
          <div className="rail-muted">
            {toolNames.length ? t("rightrail.toolCallsSoFar", { count: toolNames.length }) : t("rightrail.working")}
          </div>
        )}
      </div>
    );
  ...
  if (running) {
    return (
      <div className="rail-muted">
        {toolNames.length ? t("rightrail.workingOnTaskWith", { count: toolNames.length }) : t("rightrail.workingOnTask")}
      </div>
    );
  }
  return (
    <div className="rail-muted">
      {t("rightrail.progressPlaceholder")}
    </div>
  );
}
  ...
  return (
    <div className="artifact-viewer">
      <div className="artifact-head">
        <button className="artifact-icon-btn" onClick={onBack} aria-label={t("rightrail.backToArtifacts")} title={t("common.back")}>
        ...
        <div className="rail-actions">
          {isHtml && (
            <button
              ...
              aria-label={t("rightrail.reloadPreview")}
          ...
          {isApp && (
            <button
              ...
              aria-label={t("rightrail.openInDefaultApp")}
              title={t("rightrail.openInDefaultApp")}
            >
          ...
          <button
            ...
            title={t("rightrail.copyFullPath")}
          >
          ...
          <button
            ...
            aria-label={t("rightrail.showInFolder")}
            title={t("rightrail.showInFolder")}
          >
      ...
      <div className="artifact-preview">
        {!content ? (
          ...
          <div className="artifact-folderlist" data-testid="artifact-folder">
            ...
            {!content.entries?.length && <div className="rail-muted">{t("rightrail.folderEmpty")}</div>}
          </div>
        ) : content.kind === "office" ? (
          <div className="artifact-open-prompt">
            <Icon name="panelOpen" size={28} />
            <p>{t("rightrail.cantPreview", { type: /\.pptx?$/i.test(artifact.name) ? t("rightrail.powerpoint") : t("rightrail.word") })}</p>
            <button className="btn sm" onClick={() => revealArtifact(sessionId, artifact.path, "open")}>
              {t("rightrail.openInDefaultApp")}
            </button>
          </div>
        ) : (
    ...
    <div className="artifact-tablewrap">
      ...
      {(note || body.length > MAX_TABLE_ROWS) && (
        <div className="rail-muted artifact-table-note">
          {note}
          {body.length > MAX_TABLE_ROWS ? t("rightrail.showingFirst", { max: MAX_TABLE_ROWS, total: body.length }) : ""}
        </div>
      )}
    </div>
  );
...
function CsvTable({ text }: { text: string }) {
  ...
  if (!rows.length) return <div className="rail-muted artifact-table-note">{t("rightrail.emptyFile")}</div>;
...
function PdfViewer({ dataUrl }: { dataUrl: string }) {
  ...
  if (error) return <div className="rail-error artifact-table-note">{t("rightrail.couldNotRenderPdf", { error })}</div>;
  return (
    <div className="artifact-pdfjs">
      {loading && <div className="rail-muted artifact-table-note">{t("rightrail.renderingPdf")}</div>}
      <div ref={holder} />
    </div>
  );
}
...
function SheetViewer({ dataUrl }: { dataUrl: string }) {
  ...
  if (error) return <div className="rail-error artifact-table-note">{t("rightrail.couldNotParseSpreadsheet", { error })}</div>;
  if (!sheets) return <div className="rail-muted artifact-table-note">{t("rightrail.parsingSpreadsheet")}</div>;
  ...
  return (
    <div className="sheet-viewer">
      ...
      {sheet.rows.length ? <GridTable rows={sheet.rows} /> : <div className="rail-muted artifact-table-note">{t("rightrail.emptySheet")}</div>}
    </div>
  );
}
