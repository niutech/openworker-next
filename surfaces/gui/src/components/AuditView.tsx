    ...
    <main className="flex-1 min-w-0 flex bg-paper">
      <div className="flex-1 min-w-0 overflow-y-auto hairline-scroll">
        <div className="max-w-4xl mx-auto px-7 py-6">
          <PanelHead
            title={t("integrations.activity")}
            sub={t("audit.subtitleDetail")}
          />
    ...
    <div className={CARD + " p-3.5"}>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[11.5px] text-faint">
          {ev.connector || t("audit.fallbackTool")} · {ev.stage || ev.status || t("audit.fallbackEvent")} · {ev.timestamp}
        </span>
      </div>
      <div className="text-[11.5px] text-muted mt-0.5">
        {t("audit.sessionLabel")} {ev.session_id || "-"} {ev.approval ? `· ${ev.approval}` : ""} {ev.status ? `· ${ev.status}` : ""}
      </div>
      {ev.resource && <div className="text-[11.5px] text-faint mt-0.5">{t("audit.resourceLabel")} {ev.resource}</div>}
