...
export function PersonaView({
  ...
  useEffect(() => {
    ...
    getPersonaDetail(personaId)
      ...
      .catch(() => live && setError(t("personaView.couldNotLoad")));
    getConnectors()
  ...
  const header = (
    <div className="h-12 shrink-0 px-5 flex items-center gap-3 border-b border-line bg-paper">
      ...
      <span className="text-[13px] font-semibold">{t("personaView.title")}</span>
    </div>
  );
  ...
  if (error || !detail) {
    return (
      <main className="flex-1 min-w-0 flex flex-col bg-paper">
        {header}
        <div className="p-12 text-center text-faint text-[13px]">{error || t("common.loading")}</div>
      </main>
    );
  }
        ...
        <div className="max-w-3xl mx-auto px-7 py-6 space-y-6">
          ...
          <header className="flex items-start gap-3.5">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="text-[12px] text-muted">{detail.enabled ? t("common.enabled") : t("common.disabled")}</span>
              <Toggle checked={detail.enabled} onChange={toggleEnabled} title={t("personaView.enablePersona")} />
            </div>
          </header>
          ...
          {detail.description && (
            <section>
              <div className={`${SEC_H} mb-1.5`}>{t("personaView.about")}</div>
          ...
          {detail.tools.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-2`}>{t("personaView.builtInCapabilities")}</div>
          ...
          {detail.recommends.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1`}>{t("personaView.connectionsForFullBenefit")}</div>
              <p className="text-[12.5px] text-muted mb-2.5">
                {t("personaView.connectionsHelp", { name: shortPersonaName(detail.name, personaId) })}
              </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isMcp ? (
                            ...
                            <span className={TAG_CORE}>{t("access.core")}</span>
                          ) : null}
                        </div>
                      ...
                      {r.connected ? (
                        ...
                        >
                          {isMcp ? t("addFolder.add") : t("access.connect")}
                        </button>
                      )}
                    </div>
                  );
          ...
          {detail.default_connections.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1`}>{t("personaView.newSessionsDefault")}</div>
              <p className="text-[12.5px] text-muted mb-2.5">
                {t("personaView.defaultConnectionsHelp", { name: shortPersonaName(detail.name, personaId) })}
              </p>
                ...
                {detail.default_connections.map((c) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-[13px] font-medium">
                      ...
                      {!c.connected && (
                        <span className="text-[11px] text-faint font-normal">{t("personaView.connectToEnable")}</span>
                      )}
                    </div>
                    <Toggle
                      checked={c.enabled}
                      ...
                      title={c.connected ? t("personaView.onByDefault") : t("personaView.connectFirst")}
                    />
                  </div>
                ))}
              </div>
          ...
          <section className="flex flex-wrap gap-x-8 gap-y-2 text-[12.5px]">
            {detail.recommended_models.length > 0 && (
              <div>
                <span className="text-faint">{t("models.title")}</span> ·{" "}
            ...
            {detail.default_permission_mode && (
              <div>
                <span className="text-faint">{t("personaView.defaultMode")}</span> · {detail.default_permission_mode}
              </div>
            )}
            {detail.workspace && (
              <div>
                <span className="text-faint">{t("personaView.workspace")}</span> · {detail.workspace}
              </div>
            )}
          </section>
        </div>
