...
export function Composer(props: Props) {
  ...
  const addFiles = async (files: FileList | File[]) => {
    ...
    for (const a of read) {
      if (a.kind === "pdf" && a.data_url) {
        ...
        if (info && !info.ok) {
          showAttachNotice(
            t("composer.pdfSkipped", {
              name: a.name,
              error: info.error || t("composer.pdfReadError"),
            }),
          );
          continue;
        }
  ...
  const submit = () => {
    ...
    const msg = (skill ? text.slice(skill.length + 1) : text).trim();
    if (
      (!msg && attachments.length === 0 && !skill) ||
      props.running ||
      dictation?.recording ||
      dictationBusy
    )
    ...
    props.onSend(msg, attachments, skill);
    setText("");
    setAttachments([]);
    setPendingSkill(null);
  };
  ...
  const toggleDictation = async () => {
    ...
    try {
      if (dictation?.recording) {
        ...
        if (transcript === null) throw new Error(t("composer.transcribeError"));
        if (transcript.trim()) {
      ...
      if (!status) throw new Error(t("composer.dictationUnavailable"));
      if (!status.supported || !status.model_verified || !status.test_passed) {
        props.onConfigureVoiceInput?.();
        return;
      }
      setDictationBusy(t("composer.startingMic"));
      const recording = await startDictation();
      if (!recording?.recording) throw new Error(t("composer.micError"));
      setDictation(recording);
    } catch (error) {
      setDictationError(error instanceof Error ? error.message : t("composer.dictationUnavailable"));
  ...
  return (
    <div className="composer-wrap px-6 pb-5 pt-4">
      ...
      >
        ...
        {slashQuery !== null && (
          <div className="px-2 pt-2" data-testid="skill-popup" role="listbox" aria-label={t("composer.skills")}>
            {slashSkills === null ? (
        ...
        <div className="px-2.5 pb-2.5 pt-1 flex items-center gap-1.5">
            ...
            {attachMenuOpen && (
              <>
                ...
                <div className="absolute z-40 bottom-full mb-1 left-0 min-w-[180px] rounded-xl border border-line bg-panel shadow-2xl py-1.5">
                  {attachItem("image", t("composer.photoImage"), () => pickFiles("image/*"))}
          ...
          {dictationBusy === t("composer.transcribing") && <span className="text-[11.5px] text-accent">{t("composer.transcribing")}</span>}
          ...
          {!dictation?.recording && (needsModel ? (
            <button
              ...
              aria-label={t("composer.noModelConnected")}
            >
          ...
          {isTauri() && (
            <button
              ...
              title={
                dictationBusy ||
                (dictation?.recording
                  ? t("composer.stopRecordingTranscribe")
                  : voiceReady
                    ? t("composer.startVoiceDictation")
                    : t("composer.configureVoice"))
              }
              aria-label={dictation?.recording ? t("composer.stopDictation") : voiceReady ? t("composer.startDictation") : t("composer.configureVoice")}
          ...
          {props.running ? (
            ...
            <button
              ...
              title={needsModel ? t("composer.connectModelToSend") : undefined}
...
function UsageChip({
  ...
  const labelFor = (id: string) =>
    id === "unknown" ? t("composer.unknownModel") : modelLabels?.[id] || shortModel(id);
  ...
  return (
    <div className="relative">
      <button
        ...
        title={
          showBar
            ? t("composer.contextFullTitle", { pct, tokens: formatTokens(total) })
            : t("composer.sessionTokensTitle", { tokens: formatTokens(total) })
        }
        ...
        <>
          ...
          >
            {contextWindow ? (
              <div className="mb-2.5">
                ...
                <div className="mt-1 text-[11.5px] text-muted tabular-nums">
                  {t("composer.ofX", { fmt: formatTokens(usage.context), window: formatTokens(contextWindow), pct })}
                </div>
              </div>
            ) : usage.context > 0 ? (
              <div className="mb-2.5 text-[11.5px] text-muted tabular-nums">
                {t("composer.inContextNow", { fmt: formatTokens(usage.context) })}
              </div>
            ) : null}
            <div className="text-[10.5px] uppercase tracking-[0.06em] text-faint font-semibold mb-1">
                ...
                <div key={id}>
                  ...
                  <div className="mt-0.5 flex flex-col gap-0.5">
                    {u.cache_read + u.cache_write > 0 ? (
                      <>
                        {stat(t("composer.uncachedInput"), u.input)}
                        {stat(t("composer.cacheReads"), u.cache_read)}
                        {stat(t("composer.cacheWrites"), u.cache_write)}
                        {stat(t("composer.totalInput"), u.input + u.cache_read + u.cache_write)}
                      </>
                    ) : (
                      stat(t("composer.input"), u.input)
                    )}
                    {stat(t("composer.output"), u.output)}
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-2 pt-2 border-t border-line flex items-baseline justify-between text-[11.5px]">
              ...
              <span className="text-ink tabular-nums">{t("composer.xTokens", { fmt: formatTokens(total) })}</span>
            </div>
            {model && !modelLabels?.[model] && contextWindow === undefined && (
              <div className="mt-1 text-[10.5px] text-faint leading-snug">
                {t("composer.contextMeterUnavailable")}
              </div>
            )}
          </div>
        </>
...
function ModeMenu({
  ...
  const PERMISSION_OPTIONS: Option[] = [
    { value: "discuss", label: t("composer.modeDiscuss"), description: t("composer.modeDiscussDesc") },
    { value: "interactive", label: t("composer.modeApproval"), description: t("composer.modeApprovalDesc") },
    { value: "auto", label: t("composer.modeAuto"), description: t("composer.modeAutoDesc") },
  ];
    ...
    <div className="relative">
      ...
      <button
        ...
        title={
          ...
          (unattended ? t("composer.modeInboxSuffix") : "")
        }
      >
        {current?.label || mode}
              ...
              <>
                ...
                <div className="flex items-center gap-2 px-2.5 py-1.5">
                  <span className="flex-1 min-w-0">
                    <span className="block text-[13px] text-ink">{t("composer.sendApprovalsToInbox")}</span>
                    <span className="block text-[11px] text-faint leading-snug">
                      {t("composer.sendApprovalsDesc")}
                    </span>
                  </span>
                  <Toggle
                    checked={!!unattended}
                    onChange={onUnattendedChange}
                    title={t("composer.sendApprovalsToInboxTitle")}
                  />
                </div>
              </>
            )}
